...
from _isissl import _SSLContext, MemoryBIO, SSLSession
from _isissl import (
...
from _isissl import _DEFAULT_CIPHERS, _OPENSSL_API_VERSION
...
class TLSVersion(_IntEnum):
    MINIMUM_SUPPORTED = _isissl.PROTO_MINIMUM_SUPPORTED
    SSLv3 = _isissl.PROTO_SSLv3
    TLSv1 = _isissl.PROTO_TLSv1
    TLSv1_1 = _isissl.PROTO_TLSv1_1
    TLSv1_2 = _isissl.PROTO_TLSv1_2
    TLSv1_3 = _isissl.PROTO_TLSv1_3
    MAXIMUM_SUPPORTED = _isissl.PROTO_MAXIMUM_SUPPORTED
...
HAS_NEVER_CHECK_COMMON_NAME = hasattr(_isissl, 'HOSTFLAG_NEVER_CHECK_SUBJECT')
...
class SSLContext(_SSLContext):
    ...
    if hasattr(_isissl, 'HOSTFLAG_NEVER_CHECK_SUBJECT'):
        @property
        def hostname_checks_common_name(self):
            ncs = self._host_flags & _isissl.HOSTFLAG_NEVER_CHECK_SUBJECT
            return ncs != _isissl.HOSTFLAG_NEVER_CHECK_SUBJECT

        @hostname_checks_common_name.setter
        def hostname_checks_common_name(self, value):
            if value:
                self._host_flags &= ~_isissl.HOSTFLAG_NEVER_CHECK_SUBJECT
            else:
                self._host_flags |= _isissl.HOSTFLAG_NEVER_CHECK_SUBJECT
    else:
        @property
